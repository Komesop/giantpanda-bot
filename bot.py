python
    ...
    async def marketing_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "m_brand":
            ...
            await query.edit_message_text(txt, reply_markup=marketing_menu(), parse_mode="Markdown")
    ...
    async def personal_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "p_goals":
            ...
            if not goals:
                await query.edit_message_text("Je hebt nog geen doelen. Gebruik 'Nieuw doel' om er een toe te voegen.", reply_markup=personal_menu())
                return
            txt = "Mijn doelen:\n\n"
            for gid, title, status in goals:
                txt += f"- [{status}] {title}\n"
            await query.edit_message_text(txt, reply_markup=personal_menu(), parse_mode="Markdown")
            ...
            if not checkins:
                await query.edit_message_text("Nog geen check-ins. Start met een dagelijkse check-in.", reply_markup=personal_menu())
                return
    ...
    async def checkin_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        mood_label = {"goed": "Goed 🔥", "matig": "Matig 😐", "slecht": "Slecht 😔"}.get(mood, mood)
    
        await query.edit_message_text(
            f"Mood: {mood_label}\n\nStuur een korte notitie (of /skip om over te slaan):",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("Terug", callback_data="mode_personal_enter")],
            ]),
        )
    ...
    async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            "Gebruik /start om te beginnen of kies een optie uit het menu.",
        )
